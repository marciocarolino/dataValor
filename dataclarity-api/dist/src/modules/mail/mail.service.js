...
let MailService = MailService_1 = class MailService {
    ...
    constructor() {
        this.transporter = nodemailer_1.default.createTransport({
            ...
            auth: this.env.SMTP_USER && this.env.SMTP_PASS
                ? { user: this.env.SMTP_USER, pass: this.env.SMTP_PASS }
                : undefined,
        });
    }
    async sendEmailVerification(to, token, name) {
    ...
    `;
        ...
        try {
            ...
            this.logger.log(`E-mail de verificação enviado para ${to} [${info.messageId ?? 'sem-id'}]`);
        }
        catch (err) {
            this.logger.error(`Falha ao enviar e-mail de verificação para ${to}`, err instanceof Error ? err.message : String(err));
        }
    }
};
